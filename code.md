...
```
### 2019 entire room/apt  Spatial autocorrelation analysis
```{r}
...
```
### Local Moran's I, 2019 Airbnb entire home/apt in London
```{r}
...
head(Lward.lw$neighbours)
```
### 2020 entire room/apt  Spatial autocorrelation analysis
```{r}
...
head(Lward.lw$neighbours)
```
### Spatial autocorrelation tests result
```{r}
I_LWard_Global_Density <- points_sf_joined %>%
  pull(density) %>%
  as.vector()%>%
